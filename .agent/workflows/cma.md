...
1. First, check if `client/.env` exists. If not, inform the user to copy `client/.env.example` to `client/.env` and fill in their `OBSIDIAN_VAULT_PATH`.
2. Extract the user's question from the prompt.
3. Use the `run_command` tool to execute the following command:
```bash
python client/ask_cobalt.py "<user_question>"
```
