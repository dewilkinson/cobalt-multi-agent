...
path = r'C:\Users\rende\.gemini\antigravity\worktrees\cobalt-multi-agent\backend\src\tools\scanner.py'
...
for line in lines:
    if line.startswith('async def clear_scanner_cache() -> str:'):
        ...
        out_lines.append('    strike_list_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "data", "SCANNER_STRIKE_LIST.json"))\n')
        out_lines.append('    shield_strike_list_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "data", "SHIELD_COMBAT_LIST.json"))\n')
        ...
        out_lines.append('    for path, name in [(strike_list_path, "SCANNER_STRIKE_LIST.json"), (shield_strike_list_path, "SHIELD_COMBAT_LIST.json")]:\n')
