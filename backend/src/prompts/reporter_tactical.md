...
3. Report Architecture (MANDATORY FOR FINANCIAL QUERIES ONLY):
   - For financial execution queries (SMC, Quotes, Analysis):
     ...
     - **Institutional News & Social Sentiment (MANDATORY)**: You MUST include a dedicated section titled **"2. Institutional News & Social Sentiment"**. This section must contain a concise summary of the most relevant news of the day, **specifically citing at least 3-5 major headlines with sources and approximate timestamps (e.g., '2h ago')**. You must also provide a synthesis of social media sentiment (Reddit/Twitter). MANDATORY RULE: The news and social sentiment section should NEVER BE HALLUCINATED. If news data is not available from the tools or [SYSTEM_INJECTION] data, the entire section is to be strictly labelled 'NOT AVAILABLE' without any further commentary or placeholder text. Use bullet points and horizontal rules for clarity.
     {% else %}
