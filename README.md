...
[![DeepWiki](https://img.shields.io/badge/DeepWiki-bytedance%2Fdeer--flow-blue.svg?logo=data:image/png;base64,)](https://deepwiki.com/bytedance/cobalt-multi-agent)

<!-- DeepWiki badge generated by [https://deepwiki.ryoppippi.com/](https://deepwiki.ryoppippi.com/) -->

> Originated from Open Source, give back to Open Source.

Cobalt Multiagent (Deep Exploration and Efficient Research Flow) is a community-driven Deep Research framework that builds upon the incredible work of the open source community. Our goal is to combine language models with specialized tools for tasks like web search, crawling, and Python code execution, while giving back to the community that made this possible.

Currently, Cobalt Multiagent has officially entered the FaaS Application Center of Volcengine. Users can experience it online to intuitively feel its powerful functions and convenient operations. To meet deployment needs, Cobalt Multiagent supports one-click deployment based on Volcengine.

Please visit our official website at https://cobaltmultiagent.tech/ for more details.
...
[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/cobaltmultiagent?referralCode=-Yg50p&utm_medium=integration&utm_source=template&utm_campaign=generic)

Deploy Cobalt Multiagent quickly and easily on Railway, a cloud platform that manages your infrastructure for you. This one-click deployment provisions the Cobalt Multiagent backend, web UI, and a Postgres database to store your research results, requiring only your Google AI Studio and Tavily API keys to get started.
...
If you prefer to run Cobalt Multiagent locally, follow these simplified steps.
...
```bash
git https://github.com/rpuls/cobalt-multi-agent
...
# General Cobalt Multiagent (not tested on Railway setup)

## Supported Search Engines

### Web Search

Cobalt Multiagent supports multiple search engines that can be configured in your `.env` file using the `SEARCH_API` variable:
...
Cobalt Multiagent support private knowledgebase such as ragflow and vikingdb, so that you can use your private documents to answer questions.
...
Cobalt Multiagent implements a modular multi-agent system architecture designed for automated research and code analysis. The system is built on LangGraph, enabling a flexible state-based workflow where components communicate through a well-defined message passing system.

![Architecture Diagram](./assets/architecture.png)

> See it live at [cobaltmultiagent.tech](https://cobaltmultiagent.tech/#multi-agent-architecture)
...
Cobalt Multiagent now includes a Text-to-Speech (TTS) feature that allows you to convert research reports to speech. This feature uses the volcengine TTS API to generate high-quality audio from text. Features like speed, volume, and pitch are also customizable.
...
Cobalt Multiagent uses LangGraph for its workflow architecture. You can use LangGraph Studio to debug and visualize the workflow in real-time.

#### Running LangGraph Studio Locally

Cobalt Multiagent includes a `langgraph.json` configuration file that defines the graph structure and dependencies for the LangGraph Studio. This file points to the workflow graphs defined in the project and automatically loads environment variables from the `.env` file.
...
Cobalt Multiagent supports LangSmith tracing to help you debug and monitor your workflows. To enable LangSmith tracing:
...
```bash
docker build -t cobalt-multi-agent-api .
```

Final, start up a docker container running the web server:

```bash
# Replace cobalt-multi-agent-api-app with your preferred container name
# Start the server then bind to localhost:8000
docker run -d -t -p 127.0.0.1:8000:8000 --env-file .env --name cobalt-multi-agent-api-app cobalt-multi-agent-api

# stop the server
docker stop cobalt-multi-agent-api-app
```

### Docker Compose (include both backend and frontend)

Cobalt Multiagent provides a docker-compose setup to easily run both the backend and frontend together:
...
The following examples demonstrate the capabilities of Cobalt Multiagent:
...
Cobalt Multiagent includes a human in the loop mechanism that allows you to review, edit, and approve research plans before they are executed:
...
Cobalt Multiagent is built upon the incredible work of the open-source community. We are deeply grateful to all the projects and contributors whose efforts have made Cobalt Multiagent possible. Truly, we stand on the shoulders of giants.

We would like to extend our sincere appreciation to the following projects for their invaluable contributions:

- **[LangChain](https://github.com/langchain-ai/langchain)**: Their exceptional framework powers our LLM interactions and chains, enabling seamless integration and functionality.
- **[LangGraph](https://github.com/langchain-ai/langgraph)**: Their innovative approach to multi-agent orchestration has been instrumental in enabling Cobalt Multiagent's sophisticated workflows.
...
A heartfelt thank you goes out to the core authors of `Cobalt Multiagent`, whose vision, passion, and dedication have brought this project to life:

- **[Daniel Walnut](https://github.com/hetaoBackend/)**
- **[Henry Li](https://github.com/magiccube/)**

Your unwavering commitment and expertise have been the driving force behind Cobalt Multiagent's success. We are honored to have you at the helm of this journey.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=bytedance/cobalt-multi-agent&type=Date)](https://star-history.com/#bytedance/cobalt-multi-agent&Date)
