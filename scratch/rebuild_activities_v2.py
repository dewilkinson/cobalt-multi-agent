...
def parse_history(csv_path):
    ...
    for row in reader:
        ...
        try:
            ...
            # Parse Settlement Date (e.g. Jan-2-2026)
            date_str = row.get('Settlement Date', '').strip()
            if not date_str or date_str == '--': continue
            
            # Often T+1/T+2, but it's the best we have
...
for act in all_acts:
    ...
    iso_time = f"{date_str}T09:{(30+seconds//60)%60:02d}:{seconds%60:02d}.000Z" if seconds >= 60 else f"{date_str}T09:30:{seconds%60:02d}.000Z"
...
cache[account] = {'activities': final_activities, 'positions': cache.get(account, {}).get('positions', []), 'closed_positions': cache.get(account, {}).get('closed_positions', []), 'balances': cache.get(account, {}).get('balances', {})}
